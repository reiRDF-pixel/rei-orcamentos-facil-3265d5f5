...
ALTER TABLE public.quote_items ADD COLUMN IF NOT EXISTS marca text;
...
BEGIN
  ...
  FOR _item IN SELECT value FROM jsonb_array_elements(_items) LOOP
    IF trim(COALESCE(_item->>'descricao', '')) = '' THEN RAISE EXCEPTION 'Informe o nome do item'; END IF;
  ...
  FOR _item IN SELECT value FROM jsonb_array_elements(_items) LOOP
    ...
    INSERT INTO public.quote_items (
      quote_id, product_id, codigo, marca, descricao, quantidade, preco_unitario, desconto_percentual, total, ordem
    ) VALUES (
      _quote_id,
